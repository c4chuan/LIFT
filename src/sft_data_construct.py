...
LOCAL_MODEL_URL = "http://127.0.0.1:7451"
# 设置 OpenAI client
client = OpenAI(
...
def call_from_local(messages,num_sample = 4):
    """输入多模态 messages，调用本地模型得到回答"""
    request_data = {
        "messages": messages,
        "n": num_sample
    }
    try:
        resp = requests.post(
            f"{LOCAL_MODEL_URL}/sample", json=request_data
        ).json()
...
if __name__ == '__main__':
    ...
    cur_step = 0
    mode_provider = "Qwen" # or openai
    call_model_type = "local"
    ...
    while cur_step < total_steps:
        ...
        # 格式转换
        if mode_provider == "openai":
            batch = [messages_format_transform(batch_msgs) for batch_msgs in batch]

        # 并行获取回复，保持顺序
        if call_model_type == "api":
            responses = parallel_call_gpt_4o(batch)
        elif call_model_type == "local":
            responses = [call_from_local(batch_msg) for batch_msg in batch]
        print(responses[0])
