...
from vwa.src.envs.utils import atimeout, aretry_timeout
import requests
...
from vwa.browser_env.utils import (
    AccessibilityTree,
    DetachedPage,
    Observation,
)
...
from vwa.src.logging import time_it, atime_it
    ...
    def __init__(
        ...
        match observation_type:
            ...
            case "image_som":
                ...
                self.text_observation_type = observation_type  # type: ignore[assignment]
                self.main_observation_type = "image"
            case "image_som_without_caption":
...
class FastCachedwActionMatchingBrowserEnv(FastCachedBrowserEnv):
    ...
    def maybe_update_action_id(self, action: Action, info: dict = None) -> Action:
        ...
        if action_element_id not in action_obs_nodes_info:
            logger.debug(
                f"action_element_id={action_element_id} not found in its own nodes={action_obs_nodes_info.keys()}, skipping")
            return action
        ...
        if action_element_id in env_obs_nodes_info:
            ...
            if is_same_element(env_node, action_node):
                ...
                logger.info(
                    f"found element might have changed from {action_obs_nodes_info[action_element_id]} to {env_obs_nodes_info[action_element_id]}.")
        ...
        for i in range(error_margin + 1):
            ...
            if possible_id in env_obs_nodes_info:
                ...
                if is_same_element(env_node, action_node):
                    ...
                    action.raw_prediction = previous_raw_prediction.replace(f"[{action_element_id}]",
                                                                            f"[{possible_id}]")
