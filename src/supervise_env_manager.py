...
from src.utils.data_tools import dataset_construct,supervise_dataset_construct
...
class EnvironmentManager:
    ...
    def create_new_task(self):
        if self.task_pointer < len(self.tasks):
            ref_trajectory = self.tasks[self.task_pointer]['ref_trajectory']
            config_file = {k:v for k,v in self.tasks[self.task_pointer].items() if k != 'ref_trajectory'}
            new_task = VWATask(
            ...
            self.task_pointer += 1
            self.task_pointer = self.task_pointer % min(len(self.tasks), 200)
            return new_task
    ...
    async def parallel_produce(self,Tasks,Actions):
        ...
        for task,action,flag in zip(Tasks,Actions,flag_list):
            if flag:
                ...
                true_step_action = self.decide_action(task)
    ...
    def decide_action(self,task):
        ...
        # 边界检查: 确保ref_trajectory有足够的元素
        ref_action_position = 2 * action_index + 1
        if ref_action_position >= len(task.ref_trajectory):
            # ref_trajectory长度不足,直接返回当前动作
            return current_action

        ref_action = task.ref_trajectory[ref_action_position]

        # 3. 判断是否使用当前动作(判断函数暂时空着)
        # TODO: 实现判断函数
        should_use_current_action = self._should_use_current_action(current_action, ref_action, task)

        if should_use_current_action:
            # 使用当前动作
            return current_action
        else:
            # 使用参考动作,替换state_trajectory中的对应动作
    ...
    def _should_use_current_action(self, current_action, ref_action, task):
        ...
        Args:
            ...
            ref_action: 参考轨迹中的对应动作
            task: 当前任务对象

        Returns:
            bool: True表示使用current_action, False表示使用ref_action
        """
        # TODO: 实现具体的判断逻辑
        # 暂时返回True(总是使用当前动作)
        return True
...
initial_configs = Box({
    ...
    'annotate_path': '../data/annotate/trajectories', # 存有标注轨迹的文件夹
    'annotate_envs': 'classifieds' # 标注的环境，多个环境可以用空格分隔
})
# Instantiate manager
